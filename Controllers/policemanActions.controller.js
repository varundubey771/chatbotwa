...
module.exports.getMyCrimes = async (req, res, next) => {
  ...
  try {
    ...
    const crimeData = await findAll("crimeRegister", {
      $and: [{ investigatingOfficer: data.name }],
    });
...
module.exports.getAllCrimes = async (req, res, next) => {
  try {
    ...
    const { data } = res.locals;
    const crimeData = await findAll("crimeRegister", {
      $and: [{ investigatingOfficer: "none" }, { city: data.city }],
    });
    // console.log(crimeData)
    res.status(200).json({ crimeData });
    next();
...
module.exports.startInvestigation = async (req, res) => {
  try {
    ...
    const { data } = res.locals;
    const isInvestigationOngoing = await findOne("crimeRegister", {
      caseNo: caseNo,
      status: "ongoing",
    });
    if (isInvestigationOngoing)
      return res.status(409).send({ message: "the case is already ongoing" });
    const caseDetails = await findOne("crimeRegister", { caseNo: caseNo });
    const isCaseDataUpdated = await updateOne(
      "crimeRegister",
      { caseNo },
      { $set: { investigatingOfficer: data.name, status: "ongoing" } }
    );
    ...
    if (isCaseDataUpdated) {
      client.messages.create({
        ...
        body:
          ...
          caseNo +
          " has been started by: Inspector: " +
          data.name,
      });
      return res
        .status(200)
        .send({ message: "Case data updated successfully" });
...
module.exports.transferCase = async (req, res) => {
  ...
  const isCaseUpdated = await updateOne(
    "crimeRegister",
    { caseNo },
    { $set: { investigatingOfficer: newOfficer } }
  );
  const caseDetails = await findOne("crimeRegister", { caseNo: caseNo });
  if (isCaseUpdated) {
    // client.messages.create({
  ...
  return res
    .status(500)
    .send({
      error: "An error occured while trying to update case please try again",
    });
};
...
module.exports.registerPolice = async (req, res) => {
  ...
  if (data.userType === "policeman") {
    ...
    const doesUsernameExist = await findOne("users", {
      $or: [{ username }, { phone }],
    });
    ...
    if (doesUsernameExist) {
      res
        .status(409)
        .json({ error: `Phone number or Username already exists` });
      return;
    }
    ...
    try {
      ...
      const isUserAdded = await insertOne("users", {
        username,
        name,
        salt,
        hash,
      ...
      const token = sign(
        { username, name, city, phone, userType: "policeman" },
        "sihpolicebotsecret"
      );
...
module.exports.getPoliceMen = async (req, res) => {
  ...
  if (data.userType === "policeman") {
    try {
      const police = await findAll("users", {
        $and: [{ userType: "policeman" }],
      });
